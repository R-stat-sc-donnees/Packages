print.spMCA <- function (x, file = NULL, sep = ";", ...){
    ...
    cat("**Results of the specific Multiple Correspondence Analysis (spMCA)**\n")
    ...
    res[7, ] <- c("$var$eta2", "eta2 for variables")
    res[8, ] <- c("$var$v.contrib", "v.contrib for variables")
    res[9, ] <- c("$ind", "results for the individuals")
    res[10, ] <- c("$ind$coord", "coord. for the individuals")
